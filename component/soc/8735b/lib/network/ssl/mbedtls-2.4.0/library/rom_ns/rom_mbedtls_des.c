...
{
#ifdef RTL_HW_CRYPTO
	if (rom_ssl_ram_map.use_hw_crypto_func && rom_ssl_ram_map.hw_crypto_des_cbc_init) {
							 ...
							 const unsigned char key[MBEDTLS_DES_KEY_SIZE * 2])
{
	uint32_t sk[96];
#ifdef RTL_HW_CRYPTO
	if (rom_ssl_ram_map.use_hw_crypto_func && rom_ssl_ram_map.hw_crypto_3des_cbc_init) {

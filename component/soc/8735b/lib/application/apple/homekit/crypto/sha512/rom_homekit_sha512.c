...
void rom_sha512_finish(sha512_context *ctx, unsigned char output[64])
{
	...
	rom_sha512_update(ctx, sha512_padding, padn);
	rom_sha512_update(ctx, msglen, 16);
				...
				unsigned char output[64], int is384)
{
	...
	rom_sha512_starts(&ctx, is384);
	rom_sha512_update(&ctx, input, ilen);
	rom_sha512_finish(&ctx, output);
							...
							size_t keylen, int is384)
{
	...
	if (keylen > 128) {
		rom_sha512(key, keylen, sum, is384);
		keylen = (is384) ? 48 : 64;
		key = sum;
	}
	...
	rom_sha512_starts(ctx, is384);
	rom_sha512_update(ctx, ctx->ipad, 128);
 ...
 */
APPLE_ROM_TEXT_SECTION
void rom_sha512_hmac_update(sha512_context  *ctx,
...
void rom_sha512_hmac_finish(sha512_context *ctx, unsigned char output[64])
{
	...
	rom_sha512_finish(ctx, tmpbuf);
	rom_sha512_starts(ctx, is384);
	rom_sha512_update(ctx, ctx->opad, 128);
	rom_sha512_update(ctx, tmpbuf, hlen);
	rom_sha512_finish(ctx, output);
...
{
	rom_sha512_starts(ctx, ctx->is384);
	rom_sha512_update(ctx, ctx->ipad, 128);
}
					 ...
					 unsigned char output[64], int is384)
{
	...
	rom_sha512_hmac_starts(&ctx, key, keylen, is384);
	rom_sha512_hmac_update(&ctx, input, ilen);
	rom_sha512_hmac_finish(&ctx, output);
...
APPLE_ROM_TEXT_SECTION
static int sha512_hkdf_extract(const unsigned char *salt, size_t salt_len,
							   const unsigned char *ikm, size_t ikm_len,
							   unsigned char prk[SHA512_DIGEST_SIZE])
{
	unsigned char nullSalt[SHA512_DIGEST_SIZE];
	...
	rom_sha512_hmac(salt, salt_len, ikm, ikm_len, prk, 0);
	return 0;
}

APPLE_ROM_TEXT_SECTION
static int sha512_hkdf_expand(unsigned char prk[SHA512_DIGEST_SIZE],
							  const unsigned char *info, size_t info_len,
							  unsigned char *okm, size_t okm_len)
{
	int hash_len = SHA512_DIGEST_SIZE, N;
	unsigned char T[SHA512_DIGEST_SIZE];
	int Tlen = 0, where = 0, i;
	...
	for (i = 1; i <= N; i ++) {
		sha512_context sha_ctx;
		unsigned char c = i;

		rom_sha512_hmac_starts(&sha_ctx, prk, SHA512_DIGEST_SIZE, 0);
		rom_sha512_hmac_update(&sha_ctx, T, Tlen);
		rom_sha512_hmac_update(&sha_ctx, info, info_len);
		rom_sha512_hmac_update(&sha_ctx, &c, 1);
		rom_sha512_hmac_finish(&sha_ctx, T);
...
{
	unsigned char prk[SHA512_DIGEST_SIZE];
	return sha512_hkdf_extract(salt, salt_len, ikm, ikm_len, prk) ||
		   sha512_hkdf_expand(prk, info, info_len, okm, okm_len);
}

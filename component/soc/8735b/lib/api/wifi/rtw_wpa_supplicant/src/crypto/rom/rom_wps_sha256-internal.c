...
int rom_sha256_vector(size_t num_elem, const u8 *addr[], const size_t *len,
					  u8 *mac)
{
	struct wps_sha256_state ctx;
	int i;
	memset(&ctx, 0, sizeof(struct wps_sha256_state));
	sha256_init(&ctx);
	for (i = 0; i < num_elem; i++) {
		if (wps_sha256_process(&ctx, addr[i], len[i])) {
			return -1;
		}
	}
...
/* the K array */
//extern const unsigned long K[64];
WLAN_ROM_DATA_SECTION
static const unsigned long KK[64] = {
...
/* compress 512-bits */
WLAN_ROM_TEXT_SECTION
static int sha256_compress(struct wps_sha256_state *md, unsigned char *buf)
{
	u32 S[8], W[64], t0, t1;
	u32 t;
	int i;

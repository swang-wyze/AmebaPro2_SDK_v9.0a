...
static int fast_s_mp_sqr(mp_int *a, mp_int *b)
{
	...
	////DBG_871X("===>fast_s_mp_sqr()");
	////DBG_871X("a->used = %d,a->alloc = %d,a->sign = %d",a->used,a->alloc,a->sign);
	////DBG_871X("b->used = %d,b->alloc = %d,b->sign = %d",b->used,b->alloc,b->sign);
	...
	////DBG_871X("olduse = %d,b->used = %d",olduse,b->used);
	{
		mp_digit *tmpb;
		tmpb = b->dp;
		for (ix = 0; ix < pa; ix++) {
	...
	mp_clamp(b);
	////DBG_871X("<===fast_s_mp_sqr(),return MP_OKAY(0)");
	return MP_OKAY;
}
#endif

...
{
	return (pXComPort->poll(pXComPort->xm_port));
}

static __inline char
COMMPORT_GET(XMODEM_COM_PORT *pXComPort)
{
	return (pXComPort->get(pXComPort->xm_port));
}

s32 COMMPORT_GET_LINE_STATUS(XMODEM_COM_PORT *pXComPort)
{
	if (pXComPort->line_status != NULL) {
		return (pXComPort->line_status(pXComPort->xm_port));
	} else {
		return 0;
	}
}
...
{
	pXComPort->put(pXComPort->xm_port, c);
}
...
//            DBG_MISC_WARN("xModemGetOthers Timeout #4\r\n");
	...
	} else if (COMMPORT_GET_LINE_STATUS(pComPort) != 0) {
		pXMCtrl->rStatus = XMODEM_NAK;
		//DBG_MISC_WARN("line status err\r\n");
	} else {
		if (!is_duplicate) {
			pXMCtrl->total_frame++;
		}
	...
	if (pXMCtrl->rStatus == XMODEM_OK) {
		...
		if (is_duplicate) {
			// duplicated frame
			//DBG_MISC_WARN("Duplicated Packet!\r\n");
			pXMCtrl->rStatus = XMODEM_NAK;
		} else {
			rx_len = pXMCtrl->RxFrameHandler((char *)ptr, pXMCtrl->total_frame, pXMCtrl->expected);
			if (rx_len > 0) {
				pXMCtrl->rx_len += rx_len;
			} else {
				pXMCtrl->result += rx_len;
			}
		}
		xModemInquiry(pXMCtrl, ACK);
	} else {
...
//					rxlen -= pXMCtrl->expected;
		...
		default :
			//DBG_MISC_ERR ("XMODEM_RX_Err(0x%x)\r\n", status);
			xModemCancel(pXMCtrl);
			finished = TRUE;
			rxlen = 0;
			break;

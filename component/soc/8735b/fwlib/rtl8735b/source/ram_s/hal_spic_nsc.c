 ...
 *    1-4D-4D, 4D-4D-4Dmodes access should call this function first.
 ...
 */
SECTION_NS_ENTRY_FUNC
void NS_ENTRY spic_wait_ready_nsc(SPIC_TypeDef *spic_dev)
{
 ...
 */
SECTION_NS_ENTRY_FUNC
void NS_ENTRY spic_flush_fifo_nsc(SPIC_TypeDef *spic_dev)
{
	spic_flush_fifo(spic_dev);
}

/** \brief Description of Non-secure callable function spic_pinmux_ctl_nsc
 *
 *    spic_pinmux_ctl_nsc is used to select pinmux to operate flash.
 ...
 */
SECTION_NS_ENTRY_FUNC
hal_status_t NS_ENTRY spic_pinmux_ctl_nsc(phal_spic_adaptor_t phal_spic_adaptor, u8 ctl)
{
	return spic_pinmux_ctl(phal_spic_adaptor, ctl);
}

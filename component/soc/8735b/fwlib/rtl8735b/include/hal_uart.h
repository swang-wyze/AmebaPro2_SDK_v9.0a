...
{
#if UART_UNIFY
	puart_adapter->base_addr->UART_THR = tx_data;
#else
...
{
#if UART_UNIFY
	return (u8)(puart_adapter->base_addr->UART_RBR);
#else
...
{
#if UART_UNIFY
	puart_adapter->base_addr->UART_MCR &= (~UART_BIT_RTS);
	puart_adapter->base_addr->UART_MCR |= ((rts_ctrl == 0) ? 0 : 1) << UART_SHIFT_RTS;
#else
...
{
#if UART_UNIFY
	puart_adapter->base_addr->UART_MCR &= (~UART_BIT_SW_CTS);
	puart_adapter->base_addr->UART_MCR |= ((cts_ctrl == 0) ? 0 : 1) << UART_SHIFT_SW_CTS;
#else
...
{
#if UART_UNIFY
	return (puart_adapter->base_addr->UART_RFLVR);
#else
 ...
 */
__STATIC_INLINE
void hal_uart_tx_fifo_low_hook(phal_uart_adapter_t puart_adapter, uart_callback_t pcallback,
							   void *parg)
{
	hal_uart_stubs.hal_uart_tx_fifo_low_hook(puart_adapter, pcallback, parg);
}

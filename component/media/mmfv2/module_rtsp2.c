...
int rtsp2_control(void *p, int cmd, int arg)
{
	...
	case CMD_RTSP2_SET_PARAMS: {
		...
		if (params->type == AVMEDIA_TYPE_VIDEO) {
			...
			memcpy(&ctx->params[channel_idx], (void *)arg, sizeof(rtsp2_params_t));
			if (params->u.v.codec_id == AV_CODEC_ID_OPUS) {
				...
				if (params->u.a_opus.frame_size == 0) {
					stream_ctx->tsin_by_fs = 20 * 2 / 5;    //use 20 frame size
				} else {
					stream_ctx->tsin_by_fs = params->u.a_opus.frame_size * 2 / 5;    //divide by 2.5
				}

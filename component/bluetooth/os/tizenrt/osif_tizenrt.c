					  ...
					  void *p_param, uint16_t stack_size, uint16_t priority)
{
	...
	priority = (priority + SCHED_PRIORITY_DEFAULT > SCHED_PRIORITY_MAX ||
				priority + SCHED_PRIORITY_DEFAULT < SCHED_PRIORITY_MIN) ? SCHED_PRIORITY_DEFAULT : priority + SCHED_PRIORITY_DEFAULT;
...
void osif_timer_wrapper(void *timer)
{
	...
	if (timer_entry->timer->reload) {
		if (work_queue(LPWORK, timer_entry->timer->work_hdl, osif_timer_wrapper, (void *)(timer_entry->timer),
					   (timer_entry->timer->timeout * TICK_PER_SEC / 1000L)) != OK) {

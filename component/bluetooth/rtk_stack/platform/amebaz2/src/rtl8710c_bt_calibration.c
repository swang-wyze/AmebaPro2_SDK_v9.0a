...
#define BT_IQK_CANDIDATE_NUM			(5)
#define BT_IQK_MAX_RETRY				(2)
#define BT_DCK_TOLERANCE				(1) // Suggested by RDC BiChing
#define BT_IQK_TOLERANCE				(4) // Suggested by BT Jeffery
...
uint64_t phy_bt_iqk_lok_8710c(void *dm_void)
{
	...
	odm_set_rf_reg(dm, RF_PATH_A, 0x06, 0x000001E0, 0x5);		// PAD [8:5]
	...
	//path-A IQK setting
	odm_set_bb_reg(dm, R_0xe30, MASKDWORD, 0x1c00cc08);	//tx-tone-id[9:0] , TX-cal-M[29] , plus resultion 20200413
	...
	odm_set_rf_reg(dm, RF_PATH_A, 0x06, 0x01F, 0x0);	// TXBB [4:0]
...
uint64_t phy_bt_iqk_8710c(void *dm_void)
{
	...
	btc_set_gnt_bt_8710c(PTA_WIFI); // Disable PTA & GNT_BT to wifi
...
uint32_t bt_iqk_8710c(BT_Cali_TypeDef *cal_data, BOOLEAN store)
{
	...
	if(dev) {
		...
		if ( Hal_BT_Is_Supported(padapter) ) {
			rtl8710c_rfk_handshake((void*)padapter, true);
			result = phy_bt_iqk_8710c(&(pHalData->odmpriv));
			rtl8710c_rfk_handshake((void*)padapter, false);

...
//boolean _btiqk_lok_8721d(void *dm_void)
{
    ...
    *u32bt_iqk_x = btiqk_x;//>>16;
    *u32bt_iqk_y = btiqk_y;//>>16;
    ...
    *u32bt_dac_i = r_tx_x;//>>14;
    *u32bt_dac_q = r_tx_y;//>>4;
#endif
...
//		   u8 t)
{
	...
	while ( (!calibration_all_vaid) && (iqk_cnt < BT_IQK_MAX_RETRY) ) {
		...
		for(i=0; i < BT_IQK_CANDIDATE_NUM; i++) {
			...
			iqk_array[0][i] = btiqk_x;//result[i].IQK_xx;
			iqk_array[1][i] = btiqk_y;//result[i].IQK_yy;
			lok_array[0][i] = btlok_i;//result[i].IDAC;
			lok_array[1][i] = btlok_q;//result[i].QDAC;
#if DBG_BT_CALIBRATION
            //DBG_8195A("\n\r  lok_array   i, q may be error !!!");

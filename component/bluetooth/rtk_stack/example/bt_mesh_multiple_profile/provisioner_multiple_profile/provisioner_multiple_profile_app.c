...
(defined (CONFIG_BT_MESH_SCATTERNET) && CONFIG_BT_MESH_SCATTERNET))
	case IO_MSG_TYPE_ADV: {
		...
		if (NULL == padv_data) {
			printf("[BT Mesh Provisioner] bt_mesh_multiple_profile_peripheral_adv_timer_handler allocate padv_data fail !\r\n");
			break;
		}
...
#if defined (GAP_SCHED_TASK_GUARD) && GAP_SCHED_TASK_GUARD
void gap_sched_task_guard_func(void)
{
	printf("gap_sched_task_guard_func enter \r\n");
}
#endif
...
#endif
#if defined (GAP_SCHED_TASK_GUARD) && GAP_SCHED_TASK_GUARD
			gap_sched_task_guard_set(gap_sched_task_guard_func, 0);
#endif
		}
	}
...
void bt_mesh_provisioner_multiple_profile_app_handle_conn_param_update_evt(uint8_t conn_id, uint8_t status, uint16_t cause)
{
	...
	case GAP_CONN_PARAM_UPDATE_STATUS_SUCCESS: {
		...
		printf("bt_mesh_provisioner_multiple_profile_app_handle_conn_param_update_evt update success:conn_id %d, conn_interval 0x%x, conn_slave_latency 0x%x, conn_supervision_timeout 0x%x \r\n",
			   conn_id, conn_interval, conn_slave_latency, conn_supervision_timeout);
	}
	break;
	...
	case GAP_CONN_PARAM_UPDATE_STATUS_FAIL: {
		APP_PRINT_ERROR2("bt_mesh_provisioner_multiple_profile_app_handle_conn_param_update_evt update failed: conn_id %d, cause 0x%x",
						 conn_id, cause);
		printf("bt_mesh_provisioner_multiple_profile_app_handle_conn_param_update_evt update failed: conn_id %d, cause 0x%x\r\n",
			   conn_id, cause);
	}
	break;
	...
	case GAP_CONN_PARAM_UPDATE_STATUS_PENDING: {
		...
		printf("bt_mesh_provisioner_multiple_profile_app_handle_conn_param_update_evt update pending: conn_id %d\r\n", conn_id);
	}
	break;
...
T_APP_RESULT bt_mesh_scatternet_gap_service_callback(T_SERVER_ID service_id, void *p_para)
{
	...
	if (p_gap_data->msg_type == SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE) {
		...
		default:
			...
			//printf("bt_mesh_scatternet_gap_service_callback: unhandled msg_data.opcode 0x%x\r\n", p_gap_data->msg_data.opcode);
			break;
		}
	}
	return result;

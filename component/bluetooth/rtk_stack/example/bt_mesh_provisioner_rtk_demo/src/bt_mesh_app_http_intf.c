	...
	case RTW_BT_MESH_CMD_DELETE_NODE:
		...
		*(bt_mesh_event_value + 4) = device_number;
		...
		for (i = 0; i < device_number * 4;) {
			mesh_addr = (chartodec(*(value + i)) << 12) + (chartodec(*(value + i + 1)) << 8) + (chartodec(*(value + i + 2)) << 4) + (chartodec(*
						(value + i + 3)));//analynis length filed
			i = i + 4;
...
#if 1
			printf("Printf the httpd request there:");
...
#if 1
			printf("Printf the httpd response there:");

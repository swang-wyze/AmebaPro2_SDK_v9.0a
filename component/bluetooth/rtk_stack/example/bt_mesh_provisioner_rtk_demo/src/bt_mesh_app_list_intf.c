...
#if BT_MESH_APP_LIST_INTF_DEBUG
		printf("[BT_MESH_DEMO] Mesh Addr = 0x%x update Group Addr = 0x%x in %s()!\r\n", pbt_mesh_device_info->mesh_addr, pbt_mesh_device_info->group_addr,
			   __FUNCTION__);
#endif
	}
}
...
void bt_mesh_cmd_set_node_state_handler(void)
{
	...
	while (1) {
		...
		light_state = bt_mesh_cmd_set_node_state_api(pbt_mesh_device_info->mesh_addr, pbt_mesh_device_info->group_addr, pbt_mesh_device_info->target_light_state,
					  pbt_mesh_device_info->set_node_state_flag);
		if (light_state != 0xFF) {
			pbt_mesh_device_info->light_state = light_state;
		}
	}

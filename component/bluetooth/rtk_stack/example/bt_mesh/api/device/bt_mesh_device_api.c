...
struct mesh_task_struct   meshDeviceCmdThread;
extern CMD_MOD_INFO_S     btMeshCmdPriv;
extern INDICATION_ITEM    btMeshCmdIdPriv;

thread_return mesh_device_cmd_thread(thread_context context)
{
	/* avoid gcc compile warning */
	(void)context;
	uint8_t count = 0;
	uint32_t time_out = 2000;
	uint16_t mesh_code = MAX_MESH_DEVICE_CMD;
	...
	struct mesh_task_struct *pcmdtask = &(meshDeviceCmdThread);
		...
		if (pmeshCmdItem_s->pmeshCmdItem->meshFunc) {
			if (pmeshCmdItem_s->pmeshCmdItem->meshFunc(mesh_code, pmeshCmdItem_s)) {
				if (userApiMode == USER_API_ASYNCH) {
					...
					bt_mesh_cmdunreg(pmeshCmdItem_s);
					BT_MESH_CMD_ID_PRIV_MOD(MAX_MESH_DEVICE_CMD, NULL, USER_API_DEFAULT_MODE, 1, 0);
					goto _next;
			...
			if (os_sem_take(btMeshCmdPriv.meshThreadSema, time_out) == _FAIL) {
				...
				if (count == BT_MESH_DEV_CMD_RETRY_COUNT) {
					count = 0;
...
uint8_t bt_mesh_device_api_init(void)
{
	if (btMeshCmdPriv.meshMode == BT_MESH_DEVICE) {
		printf("[BT_MESH] %s(): MESH DEVICE is already running !\r\n", __func__);
		return 1;
	} else if (btMeshCmdPriv.meshMode == BT_MESH_PROVISIONER) {
		printf("[BT_MESH] %s(): MESH PROVISIONER is running, need deinitialization firstly !\r\n", __func__);
		return 1;
	}
	...
	BT_MESH_CMD_ID_PRIV_MOD(MAX_MESH_DEVICE_CMD, NULL, USER_API_DEFAULT_MODE, 1, 0);
	meshDeviceCmdThread.blocked = 0;
	os_sem_create(&meshDeviceCmdThread.wakeup_sema, 0, 0xffffffff);
	os_sem_create(&meshDeviceCmdThread.terminate_sema, 0, 0xffffffff);
	if (os_task_create(&meshDeviceCmdThread.task, "mesh_device_cmd_thread", mesh_device_cmd_thread,
					   NULL, 1024, 3) != true) {
		meshDeviceCmdThread.blocked = 1;
		os_sem_delete(meshDeviceCmdThread.wakeup_sema);
		os_sem_delete(meshDeviceCmdThread.terminate_sema);
...
void bt_mesh_device_api_deinit(void)
{
	...
	os_sem_take(meshDeviceCmdThread.terminate_sema, 0xFFFFFFFF);

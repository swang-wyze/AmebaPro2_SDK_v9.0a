...
static bool light_cwrgb_client_receive(mesh_msg_p pmesh_msg)
{
	...
	case MESH_MSG_LIGHT_CWRGB_STAT:
		if (pmesh_msg->msg_len == sizeof(light_cwrgb_stat_t)) {
			...
			printf("light 0x%04x cwrgb %d-%d-%d-%d-%d\r\n", pmesh_msg->src, pmsg->cwrgb[0],
		...
		uint32_t type, void *pargs)
{
	...
	case LIGHT_LIGHTNESS_CLIENT_STATUS_LAST: {
		...
		printf("light lightness client receive: src %d, lightness %d\r\n", pdata->src,
			   pdata->lightness);
#if defined(CONFIG_BT_MESH_USER_API) && CONFIG_BT_MESH_USER_API
	...
	case LIGHT_LIGHTNESS_CLIENT_STATUS_RANGE: {
		...
		printf("light lightness client receive: src %d, status %d, min %d, max %d\r\n",
									 ...
									 uint32_t type, void *pargs)
{
	...
	case LIGHT_CTL_CLIENT_STATUS_DEFAULT: {
		...
		printf("light ctl client receive: src  %d, lightness %d, temperature %d, delta_uv %d\r\n",
									 ...
									 uint32_t type, void *pargs)
{
	...
	case LIGHT_HSL_CLIENT_STATUS_DEFAULT: {
		...
		printf("light hsl client receive: src %d, lightness %d, hue %d, saturation %d\r\n",
			   pdata->src, pdata->lightness, pdata->hue, pdata->saturation);
#if defined(CONFIG_BT_MESH_USER_API) && CONFIG_BT_MESH_USER_API
	...
	case LIGHT_HSL_CLIENT_STATUS_RANGE: {
		...
		printf("light hsl client receive: src %d, status %d, hue min %d, hue max %d, saturation min %d, saturation max %d\r\n",
									 ...
									 uint32_t type, void *pargs)
{
	...
	case LIGHT_XYL_CLIENT_STATUS: {
		...
		if (pdata->optional) {
			printf("light xyl client receive: src %d, present lightness %d, present xyl_x %d, present xyl_y %d, remain time step(%d) resolution(%d)\r\n",
				   pdata->src,
			...
			printf("light xyl client receive: src %d, present lightness %d, present hue %d, presnet saturation %d\r\n",
	...
	case LIGHT_XYL_CLIENT_STATUS_TARGET: {
		light_xyl_client_status_t *pdata = pargs;
		if (pdata->optional) {
			printf("light xyl client receive: src %d, target lightness %d, target xyl_x %d, target xyl_y %d, remain time step(%d) resolution(%d)\r\n",
				   pdata->src,
				   pdata->xyl.xyl_lightness,
				   pdata->xyl.xyl_x, pdata->xyl.xyl_y,
				   pdata->remaining_time.num_steps,
			...
			printf("light xyl client receive: src %d, target lightness %d, target xyl_x %d, target xyl_y %d\r\n",
				   pdata->src, pdata->xyl.xyl_lightness, pdata->xyl.xyl_x, pdata->xyl.xyl_y);
		}
#if defined(CONFIG_BT_MESH_USER_API) && CONFIG_BT_MESH_USER_API
	...
	case LIGHT_XYL_CLIENT_STATUS_DEFAULT: {
		...
		printf("light xyl client receive: src %d, lightness %d, xyl_x %d, xyl_y %d\r\n",
			   pdata->src, pdata->xyl.xyl_lightness, pdata->xyl.xyl_x, pdata->xyl.xyl_y);
#if defined(CONFIG_BT_MESH_USER_API) && CONFIG_BT_MESH_USER_API
	...
	case LIGHT_XYL_CLIENT_STATUS_RANGE: {
		...
		printf("light xyl client receive: src %d, status %d, xyl_x min %d, xyl_x max %d, xyl_y min %d, xyl_y max %d\r\n",
									...
									uint32_t type, void *pargs)
{
	...
	case LIGHT_LC_CLIENT_MODE_STATUS: {
		...
		printf("light lc client receive: src %d, mode %d\r\n", pdata->src, pdata->mode);
	}
	break;
	case LIGHT_LC_CLIENT_OM_STATUS: {
		light_lc_client_om_status_t *pdata = pargs;
		printf("light lc client receive: src %d, occupancy mode %d\r\n", pdata->src, pdata->mode);
	}
	break;
	case LIGHT_LC_CLIENT_LIGHT_ON_OFF_STATUS: {
		light_lc_client_light_on_off_status_t *pdata = pargs;
		if (pdata->optional) {
			printf("light lc client receive: src %d, present on off %d, target on off %d, remain time step(%d) resolution(%d)\r\n",
				   pdata->src,
			...
			printf("light lc client receive: src %d, present on off %d\r\n",
	...
	case LIGHT_LC_CLIENT_PROPERTY_STATUS: {
		...
		printf("light lc client receive: src %d, property id 0x%x, property value %d\r\n",

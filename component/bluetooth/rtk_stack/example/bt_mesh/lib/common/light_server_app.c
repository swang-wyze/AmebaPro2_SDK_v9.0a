...
static int32_t lighting_lightness_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
	...
	case LIGHT_LIGHTNESS_SERVER_GET: {
		...
		printf("light lightness server receive: lightness %d \r\n", cur_lightness);
	}
	break;
	case LIGHT_LIGHTNESS_SERVER_GET_LINEAR: {
		...
		printf("light lightness server receive: linear lightness %d \r\n", cur_linear_lightness);
	}
	break;
	case LIGHT_LIGHTNESS_SERVER_GET_DEFAULT: {
		...
		printf("light lightness server receive: default lightness %d \r\n", default_lightness);
	}
	break;
	case LIGHT_LIGHTNESS_SERVER_GET_LAST: {
		...
		printf("light lightness server receive: last lightness %d \r\n", last_lightness);
	}
	break;
	case LIGHT_LIGHTNESS_SERVER_GET_RANGE: {
		...
		printf("light lightness server receive: lightness_max %d, lightness_min %d \r\n",
	...
	case LIGHT_LIGHTNESS_SERVER_SET: {
		...
		printf("light lightness server receive: set lightness %d, total_time %d, remaining_time %d \r\n",
	...
	case LIGHT_LIGHTNESS_SERVER_SET_LINEAR: {
		...
		printf("light lightness server receive: set linear lightness %d, total_time %d, remaining_time %d \r\n",
	...
	case LIGHT_LIGHTNESS_SERVER_SET_LAST: {
		...
		printf("light lightness server receive: set last lightness %d \r\n", last_lightness);
	}
	break;
	default:
		break;
...
static int32_t lighting_lightness_setup_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
	...
	case LIGHT_LIGHTNESS_SERVER_SET_DEFAULT: {
		...
		printf("light lightness server receive: set default lightness %d \r\n", default_lightness);
	}
	break;
	case LIGHT_LIGHTNESS_SERVER_SET_RANGE: {
		...
		printf("light lightness server receive: set lightness_max %d, lightness_min %d \r\n",
			   lightness_range.range_max,
			   lightness_range.range_min);
	}
	break;
...
static int32_t lighting_ctl_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
	...
	case LIGHT_CTL_SERVER_GET: {
		...
		printf("lighting ctl server receive: lightness %d, temperature %d \r\n",
	...
	case LIGHT_CTL_SERVER_GET_DEFAULT: {
		...
		printf("lighting ctl server receive: lightness %d, temperature %d, delta_uv %d \r\n",
	...
	case LIGHT_CTL_SERVER_GET_TEMPERATURE_RANGE: {
		...
		printf("lighting ctl server receive: temperature range_max %d, range_min %d \r\n",
	...
	case LIGHT_CTL_SERVER_SET: {
		...
		printf("lighting ctl server receive: set delta_uv %d, lightness %d, remaining_time %d, temperature %d, total_time %d \r\n",
...
static int32_t lighting_ctl_temperature_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
	...
	case LIGHT_CTL_SERVER_GET_TEMPERATURE: {
		...
		printf("lighting ctl temperature server receive: set temperature %d, delta_uv %d \r\n",
	...
	case LIGHT_CTL_SERVER_SET_TEMPERATURE: {
		...
		printf("lighting ctl temperature server receive: set temperature %d, delta_uv %d, total_time %d, remaining_time %d \r\n",
...
static int32_t lighting_ctl_setup_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
	...
	case LIGHT_CTL_SERVER_SET_TEMPERATURE_RANGE: {
		...
		printf("lighting ctl setup server receive: set temperature range_max %d, range_min %d \r\n",
	...
	case LIGHT_CTL_SERVER_SET_DEFAULT: {
		...
		printf("lighting ctl setup server receive: set default delta_uv %d, lightness %d, temperature %d \r\n",
...
static int32_t lighting_hsl_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
	...
	case LIGHT_HSL_SERVER_GET: {
		...
		printf("lighting hsl server receive: hue %d, lightness %d, saturation %d \r\n",
			   cur_light_hsl.hue,
	...
	case LIGHT_HSL_SERVER_GET_DEFAULT: {
		...
		printf("lighting hsl server receive: default hue %d, lightness %d, saturation %d \r\n",
	...
	case LIGHT_HSL_SERVER_GET_RANGE: {
		...
		printf("lighting hsl server receive: hue_range_min %d, hue_range_max %d, saturation_range_min %d, saturation_range_max %d \r\n",
	...
	case LIGHT_HSL_SERVER_SET: {
		...
		printf("lighting hsl server receive: set hue %d, lightness %d, remaining_time %d, saturation %d, total_time %d \r\n",
...
static int32_t lighting_hsl_hue_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
	...
	case LIGHT_HSL_SERVER_GET_HUE: {
		...
		printf("lighting hsl hue server receive: hue %d \r\n", cur_light_hsl_hue.hue);
	}
	break;
	case LIGHT_HSL_SERVER_SET_HUE: {
		...
		printf("lighting hsl hue server receive: set hue %d, remaining_time %d, total_time %d \r\n",
...
static int32_t lighting_hsl_saturation_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
	...
	case LIGHT_HSL_SERVER_GET_SATURATION: {
		...
		printf("lighting hsl saturation server receive: saturation %d \r\n", cur_light_hsl_saturation.saturation);
	}
	break;
	case LIGHT_HSL_SERVER_SET_SATURATION: {
		...
		printf("lighting hsl saturation server receive: set saturation %d, remaining_time %d, total_time %d \r\n",
...
static int32_t lighting_hsl_setup_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
	...
	case LIGHT_HSL_SERVER_SET_DEFAULT: {
		...
		printf("lighting hsl setup server receive: set default lightness %d, hue %d, saturation %d \r\n",
	...
	case LIGHT_HSL_SERVER_SET_RANGE: {
		...
		printf("lighting hsl setup server receive: set hue_range_min %d, hue_range_max %d, saturation_range_min %d, saturation_range_max %d \r\n",
...
static int32_t lighting_xyl_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
	...
	case LIGHT_XYL_SERVER_GET: {
		...
		printf("lighting xyl server receive: xyl_lightness %d, xyl_x %d, xyl_y %d \r\n",
	...
	case LIGHT_XYL_SERVER_GET_DEFAULT: {
		...
		printf("lighting xyl server receive: default xyl_lightness %d, xyl_x %d, xyl_y %d \r\n",
	...
	case LIGHT_XYL_SERVER_GET_RANGE: {
		...
		printf("lighting xyl server receive: xyl_x_range_max %d, xyl_x_range_min %d, xyl_y_range_max %d, xyl_y_range_min %d \r\n",
	...
	case LIGHT_XYL_SERVER_SET: {
		...
		printf("lighting xyl server receive: set xyl %d, total_time %d, remaining_time %d \r\n",
...
static int32_t lighting_xyl_setup_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
	...
	case LIGHT_XYL_SERVER_SET_RANGE: {
		...
		printf("lighting xyl setup server receive: set xyl_x_range_max %d, xyl_x_range_min %d, xyl_y_range_max %d, xyl_y_range_min %d \r\n",

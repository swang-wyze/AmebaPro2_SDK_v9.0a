							  ...
							  uint32_t type, void *pargs)
{
	...
	case DATATRANS_SERVER_WRITE: {
		...
		printf("remote write %d bytes: ", pdata->data_len);
		mesh_data_dump(pdata->data, pdata->data_len);
	}
	break;
	...
	case DATATRANS_CLIENT_STATUS: {
		...
		printf("written %d bytes, status: %d\r\n", pdata->written_len, pdata->status);
#if defined(CONFIG_BT_MESH_USER_API) && CONFIG_BT_MESH_USER_API
		if (!pdata->status) {
	...
	case DATATRANS_CLIENT_DATA: {
		...
		printf("read %d data from remote: ", pdata->data_len);
		mesh_data_dump(pdata->data, pdata->data_len);
#if defined(CONFIG_BT_MESH_USER_API) && CONFIG_BT_MESH_USER_API
		if (pdata->data_len) {

									...
									void *pargs)
{
	...
	case RMT_PROV_CLIENT_SCAN_CAPS_STATUS: {
		...
		printf("rmt_prov_scan_caps_status: src 0x%04x, max scanned items %d, support active scan %d\r\n",
	...
	case RMT_PROV_CLIENT_SCAN_STATUS: {
		...
		printf("rmt_prov_scan_status: src 0x%04x, status %d, scan state %d, scanned items limit %d, timeout %d\r\n",
	...
	case RMT_PROV_CLIENT_SCAN_REPORT: {
		...
		printf("rmt_prov_scan_report: rssi %d, oob %d, uri hash %d, uuid ",
			   pdata->rssi, oob, uri_hash);
		mesh_data_dump(pdata->uuid, 16);
	}
	break;
	case RMT_PROV_CLIENT_EXTENED_SCAN_REPORT: {
		...
		if (pdata->adv_structs_len > 0) {
			...
			mesh_data_dump(pdata->padv_structs, pdata->adv_structs_len);
		}
	}
	break;
	case RMT_PROV_CLIENT_LINK_STATUS: {
		rmt_prov_client_link_status_t *pdata = (rmt_prov_client_link_status_t *)pargs;
		printf("rmt_prov_link_status: src 0x%04x, status %d, link state %d\r\n",
	...
	case RMT_PROV_CLIENT_LINK_REPORT: {
		...
		printf("rmt_prov_link_report: src 0x%04x, status %d, link state %d, reason %d\r\n",

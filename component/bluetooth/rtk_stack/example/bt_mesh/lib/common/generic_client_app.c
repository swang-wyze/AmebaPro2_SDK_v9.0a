		...
		uint32_t type, void *pargs)
{
	...
	case GENERIC_DEFAULT_TRANSITION_TIME_CLIENT_STATUS: {
		...
		printf("gdtt client receive: src = %d, steps = %d, resolution = %d\r\n",
		...
		uint32_t type, void *pargs)
{
	...
	case GENERIC_LEVEL_CLIENT_STATUS: {
		...
		if (pdata->optional) {
			printf("genric level client receive: src = %d, present = %d, target = %d, remain time = step(%d), \
resolution(%d)\r\n", pdata->src, pdata->present_level, pdata->target_level,
				   pdata->remaining_time.num_steps,
				   pdata->remaining_time.step_resolution);
		} else {
			printf("generic level client receive: src = %d, present = %d\r\n", pdata->src,
				   pdata->present_level);
		}
	}
	break;
		...
		uint32_t type, void *pargs)
{
	...
	case GENERIC_BATTERY_CLIENT_STATUS: {
		...
		printf("genric battery client receive: src = %d, battery level = %d, time to discharge = %d, \
		...
		uint32_t type, void *pargs)
{
	...
	case GENERIC_POWER_ON_OFF_CLIENT_STATUS: {
		...
		printf("generic power on/off receive: src = %d, power = %d\r\n", pdata->src,
			   pdata->on_power_up);
	}
	break;
	default:
		...
		uint32_t type, void *pargs)
{
	...
	case GENERIC_POWER_LEVEL_CLIENT_STATUS: {
		...
		if (pdata->optional) {
			printf("genric power level client receive: src = %d, present = %d, target = %d, remain time = step(%d), \
resolution(%d)\r\n", pdata->src, pdata->present_power, pdata->target_power,
				   pdata->remaining_time.num_steps,
				   pdata->remaining_time.step_resolution);
		} else {
			printf("generic power level client receive: src = %d, present = %d\r\n", pdata->src,
				   pdata->present_power);
		}
	}
	break;
	case GENERIC_POWER_LEVEL_CLIENT_STATUS_LAST: {
		...
		printf("generic power level client receive: power = %d\r\n", pdata->power);
	}
	break;
	...
	case GENERIC_POWER_LEVEL_CLIENT_STATUS_RANGE: {
		...
		printf("generic power level client receive: status = %d, min = %d, max = %d\r\n",
								  ...
								  uint32_t type, void *pargs)
{
	...
	case HEALTH_CLIENT_STATUS_REGISTERED: {
		...
		printf("health client receive: src = %d, test id = %d, company id = %d, fault = ",
			   pdata->src,
			   pdata->test_id, pdata->company_id);
		for (int i = 0; i < pdata->fault_array_len; ++i) {
			printf("%d ", pdata->fault_array[i]);
		}
		printf("\r\n");
	}
	break;
								...
								uint32_t type, void *pargs)
{
	...
	case TIME_CLIENT_STATUS: {
		...
		printf("time client receive: src = %d, tai_seconds = 0x%02x%02x%02x%02x%02x, \
...
time_zone_offset = %d\r\n", pdata->src, pdata->tai_time.tai_seconds[4],
	...
	case TIME_CLIENT_STATUS_ZONE: {
		...
		printf("time client receive: time_zone_offset_current = %d, \
time_zone_offset_new = %d, tai_of_zone_change = 0x%02x%02x%02x%02x%02x\r\n",
	...
	case TIME_CLIENT_STATUS_TAI_UTC_DELTA: {
		...
		printf("time client receive: tai_utc_delta_current = %d, tai_utc_delta_new = %d, \
tai_of_delta_change = 0x%02x%02x%02x%02x%02x\r\n", pdata->tai_utc_delta_current,
	...
	case TIME_CLIENT_STATUS_ROLE: {
		...
		printf("time client receive: role = %d\r\n", pdata->role);
#if defined(CONFIG_BT_MESH_USER_API) && CONFIG_BT_MESH_USER_API
								 ...
								 uint32_t type, void *pargs)
{
	...
	case SCENE_CLIENT_STATUS: {
		...
		if (pdata->optional) {
			printf("scene client receive: src = %d, status = %d, current = %d, target = %d, \
remain time = step(%d), resolution(%d)\r\n", pdata->src, pdata->status, pdata->current_scene,
				   pdata->target_scene,
				   pdata->remaining_time.num_steps, pdata->remaining_time.step_resolution);
		} else {
			printf("scene client receive: status = %d, current = %d\r\n", pdata->status,
				   pdata->current_scene);
		}
	...
	case SCENE_CLIENT_REGISTER_STATUS: {
		...
		printf("scene client receive: status = %d, current scene = %d, scene = ", pdata->status,
			   pdata->current_scene);
		for (int i = 0; i < pdata->scene_array_len; ++i) {
			printf("%d ", pdata->scene_array[i]);
		}
		printf("\r\n");
								  ...
								  uint32_t type, void *pargs)
{
	...
	case SENSOR_CLIENT_STATUS_DESCRIPTOR: {
		...
		if (0 == pdata->num_descriptors) {
			printf("sensor client receive: property id = %d\r\n", pdata->property_id);
		} else {
			for (uint16_t i = 0; i < pdata->num_descriptors; ++i) {
				printf("sensor client receive: property id = %d, positive rolerance = %d, negative tolerance = %d, sampling function = %d, measurement period = %d, updata_interval = %d\r\n",
	...
	case SENSOR_CLIENT_STATUS_CADENCE: {
		...
		printf("sensor client receive: src = %d, property id = %d, divisor = %d, trigger type = %d, ",
			   pdata->src,
			   pdata->property_id,
		...
		if (SENSOR_TRIGGER_TYPE_CHARACTERISTIC == pdata->cadence->status_trigger_type) {
			printf("trigger delta down = ");
			mesh_data_dump((uint8_t *)pdata->cadence->status_trigger_delta_down, pdata->cadence->raw_value_len);
			printf(", ");
			printf("trigger delta up = ");
			mesh_data_dump((uint8_t *)pdata->cadence->status_trigger_delta_up, pdata->cadence->raw_value_len);
			printf(", ");
		} else {
			printf("trigger delta down = ");
			mesh_data_dump((uint8_t *)pdata->cadence->status_trigger_delta_down, 2);
			printf(", ");
			printf("trigger delta up = ");
			mesh_data_dump((uint8_t *)pdata->cadence->status_trigger_delta_up, 2);
			printf(", ");
		}

		printf("min interval = %d, ", pdata->cadence->status_min_interval);

		printf("fast cadence low = ");
		mesh_data_dump((uint8_t *)pdata->cadence->fast_cadence_low, pdata->cadence->raw_value_len);
		printf(", ");
		printf("fast cadence high = ");
		mesh_data_dump((uint8_t *)pdata->cadence->fast_cadence_high, pdata->cadence->raw_value_len);
	}
	break;
	case SENSOR_CLIENT_STATUS_SETTINGS: {
		sensor_client_status_settings_t *pdata = pargs;
		printf("sensor client receive: property id = %d, setting ids = ", pdata->property_id);
		for (uint16_t i = 0; i < pdata->num_ids; ++i) {
			printf("0x%x ", pdata->setting_ids[i]);
		}
		printf("\r\n");
	}
	break;
	case SENSOR_CLIENT_STATUS_SETTING: {
		...
		printf("sensor client receive: src = %d, property id = %d, setting property id = %d",
			   pdata->src,
			   pdata->property_id,
			   pdata->setting_property_id);
		if (NULL != pdata->setting) {
			printf(", access = %d", pdata->setting->setting_access);
			if (pdata->setting->setting_raw_len > 0) {
				printf(", setting raw = ");
				mesh_data_dump((uint8_t *)pdata->setting->setting_raw, pdata->setting->setting_raw_len);
			}
		}
		printf("\r\n");
	}
	...
	case SENSOR_CLIENT_STATUS: {
		...
		printf("sensor client receive: marshalled sensor data src = %d, ", pdata->src);
		mesh_data_dump((uint8_t *)pdata->marshalled_sensor_data, pdata->marshalled_sensor_data_len);
	}
	break;
	case SENSOR_CLIENT_STATUS_COLUMN: {
		sensor_client_status_column_t *pdata = pargs;
		printf("sensor client receive: property id = %d, column raw data = ", pdata->property_id);
		mesh_data_dump((uint8_t *)pdata->column_raw_value, pdata->column_raw_value_len);
	}
	break;
	case SENSOR_CLIENT_STATUS_SERIES: {
		sensor_client_status_series_t *pdata = pargs;
		printf("sensor client receive: property id = %d, series raw data = ", pdata->property_id);
		mesh_data_dump((uint8_t *)pdata->series_raw_value, pdata->series_raw_value_len);
	}
	break;
	default:
		break;
									 ...
									 uint32_t type, void *pargs)
{
	...
	case SCHEDULER_CLIENT_STATUS: {
		...
		printf("scheduler client receive: schedulers = %d\r\n", pdata->schedulers);
	...
	case SCHEDULER_CLIENT_STATUS_ACTION: {
		...
		printf("scheduler client receive: src = %d, index = %d, year = %d, month = %d, day = %d, hour = %d, minute = %d, second = %d, \
		...
		uint32_t type, void *pargs)
{
	...
	case GENERIC_LOCATION_CLIENT_STATUS_GLOBAL: {
		...
		printf("generic location client receive: src = %d, latitude = %d, longitude = %d, altitude = %d\r\n",
			   pdata->src,
	...
	case SCHEDULER_CLIENT_STATUS_ACTION: {
		...
		printf("generic location client receive: src = %d, north = %d, east = %d, altitude = %d, floor = %d, stationary = %d, updata time = %d, precision = %d\r\n",
		...
		uint32_t type, void *pargs)
{
	...
	case GENERIC_USER_PROPERIES_CLIENT_STATUS: {
		...
		printf("generic property client receive: src = %d", pdata->src);
		if (pdata->num_ids > 0) {
			printf(", user property ids = ");
			for (uint8_t i = 0; i < pdata->num_ids; ++i) {
				printf("%x ", pdata->pproperty_ids[i]);
			}
		}
		printf("\r\n");
	}
	...
	case GENERIC_USER_PROPERTY_CLIENT_STATUS: {
		...
		printf("generic property client receive: src = %d,  property id = 0x%x, property access = %d",
		...
		if (pdata->value_len > 0) {
			...
			for (uint8_t i = 0; i < pdata->value_len; ++i) {
				printf("%d ", pdata->pproperty_value[i]);
			}
		}
		printf("\r\n");
	}
	...
	case GENERIC_MANUFACTURER_PROPERTIES_CLIENT_STATUS: {
		...
		if (pdata->num_ids > 0) {
			...
			for (uint8_t i = 0; i < pdata->num_ids; ++i) {
				printf("%4x ", pdata->pproperty_ids[i]);
			}
		}
		printf("\r\n");
	}

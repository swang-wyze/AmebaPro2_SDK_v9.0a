...
bool blob_transfer_client_receive(mesh_msg_p pmesh_msg)
{
	...
	case MESH_MSG_BLOB_TRANSFER_STATUS: {
		...
		if (pmesh_msg->msg_len == MEMBER_OFFSET(blob_transfer_status_t, blob_id)) {
			printf("receive blob transfer status: status %d, mode %d, phase %d\r\n",
			...
			printf("receive blob transfer status: status %d, mode %d, phase %d, blob_id \r\n",
				   pmsg->status, pmsg->transfer_mode, pmsg->transfer_phase);
			mesh_data_dump(pmsg->blob_id, 8);
		} else {
			memcpy(status_data.blob_id, pmsg->blob_id, 8);
			...
			printf("receive blob transfer status: status %d, mode %d, phase %d, blob_size %d, block_size_log %d, transfer_mtu_size %d, blob_id \r\n",
			...
			if (pmesh_msg->msg_len > sizeof(blob_transfer_status_t)) {
				...
				printf("blocks_not_received = ");
				mesh_data_dump((uint8_t *)pmissing_blocks, missing_blocks_len * 2);
			}
		}
	...
	case MESH_MSG_BLOB_BLOCK_STATUS: {
		...
		printf("receive blob block status: status %d, format %d, block_num %d, chunk_size %d",
		...
		if (pmsg->format == BLOB_CHUNK_MISSING_FORMAT_ENCODED) {
			...
			if (data_len > 0) {
				...
				mesh_data_dump((uint8_t *)pmissing_chunks, missing_chunks_len * 2);
			}
	...
	case MESH_MSG_BLOB_PARTIAL_BLOCK_REPORT: {
		...
		printf("receive partial block report: encoded missing chunks = ");
	...
	case MESH_MSG_BLOB_INFO_STATUS: {
		...
		printf("receive blob info status: min_block_size_log %d, max_block_size_log %d, max_total_chunks %d, max_chunk_size %d, max_blob_size %d, server_mtu_size %d, supported_transfer_mode %d\r\n",

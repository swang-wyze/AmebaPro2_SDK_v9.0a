...
bool fw_dist_client_receive(mesh_msg_p pmesh_msg)
{
	...
	case MESH_MSG_FW_DIST_RECVS_STATUS: {
		...
		printf("receive fw dist recivers status: src 0x%04x, status %d, list count %d\r\n",
	...
	case MESH_MSG_FW_DIST_RECVS_LIST: {
		...
		printf("receive fw dist receivers list: src 0x%04x, recvs_list_cnt %d, first_index %d\r\n",
		...
		for (uint8_t i = 0;
			...
			printf("addr 0x%04x, retrieved_update_phase %d, update_status %d, transfer_status %d, transfer_progress %d, update_fw_image_idx %d\r\n",
	...
	case MESH_MSG_FW_DIST_CAPS_STATUS: {
		...
		printf("receive fw dist capabilites: src 0x%04x max_dist_recvs_list_size %d, max_fw_images_list_size %d, max_fw_image_size %d, max_upload_spcace %d, remaining_upload_space %d, oob_retrieval_supported %d, supported_uri_scheme_names = \r\n",
		...
		mesh_data_dump(pmsg->supported_uri_scheme_names,
	...
	case MESH_MSG_FW_DIST_STATUS: {
		...
		printf("receive fw dist status: src 0x%04x, status %d, phase %d, dist_multicast_addr 0x%04x, dist_appkey_index 0x%04x, dist_ttl %d, dist_timeout_base %d, dist_transfer_mode %d, update_policy %d, dist_fw_image_idx %d\r\n",
	...
	case MESH_MSG_FW_DIST_UPLOAD_STATUS: {
		...
		printf("receive fw dist upload status: src 0x%04x, status %d, phase %d, upload_progress %d, upload_fw_id = ",
			   pmesh_msg->src, pmsg->status, pmsg->phase, pmsg->upload_progress);
		mesh_data_dump(pmsg->upload_fw_id, pmesh_msg->msg_len - sizeof(fw_dist_upload_status_t));
	}
	break;
	case MESH_MSG_FW_DIST_FW_STATUS: {
		...
		printf("receive fw dist fw status: src 0x%04x, status %d, entry_cnt %d, dw_fw_image_idx %d, fw_id = ",
			   pmesh_msg->src, pmsg->status, pmsg->entry_cnt, pmsg->dist_fw_image_idx);
		mesh_data_dump(pmsg->fw_id, pmesh_msg->msg_len - sizeof(fw_dist_fw_status_t));
	}
	break;
	default:
		ret = FALSE;

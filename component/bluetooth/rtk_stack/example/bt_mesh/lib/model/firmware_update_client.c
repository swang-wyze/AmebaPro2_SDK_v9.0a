...
bool fw_update_client_receive(mesh_msg_p pmesh_msg)
{
	...
	case MESH_MSG_FW_UPDATE_INFO_STATUS: {
		...
		printf("receive fw update info status: src 0x%04x, fw_info_list_count %d, first_index %d\r\n",
		...
		if (fw_info_cnt > 0) {
			...
			for (uint8_t i = 0; i < fw_info_cnt; ++i) {
				...
				printf("    firmware id 0x");
				mesh_data_dump(pdata, pfw_info[i].fw_id_len);
				pdata += pfw_info[i].fw_id_len;
				pfw_info[i].update_uri_len = *pdata ++;
				memcpy(pfw_info[i].update_uri, pdata, pfw_info[i].update_uri_len);
				printf("    update uri 0x");
				mesh_data_dump(pdata, pfw_info[i].update_uri_len);
	...
	case MESH_MSG_FW_UPDATE_FW_METADATA_STATUS:
		if (pmesh_msg->msg_len == sizeof(fw_update_fw_metadata_status_t)) {
			...
			printf("receive fw metadata status: src 0x%04x, status %d, addi_info %d, fw_image_idx %d\r\n",
	...
	case MESH_MSG_FW_UPDATE_STATUS: {
		...
		if (pmesh_msg->msg_len == sizeof(fw_update_status_t)) {
			...
			printf("receive fw update status: src 0x%04x, status %d, update_phase %d, update_ttl %d, addi_info %d, update_timeout_base %d, update_fw_image_idx %d, blob_id 0x",
			...
			printf("receive fw update status: src 0x%04x, status %d, update_phase %d\r\n", pmsg->status,

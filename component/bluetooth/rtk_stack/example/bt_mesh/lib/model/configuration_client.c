			...
			while (pdata < pend) {
				...
				if (vendor_model_num) {
					...
					for (model_num = 0; model_num < vendor_model_num; model_num++, pdata += 4) {
						printf(" 0x%08x", LE_EXTRN2DWORD(pdata));
					}
					printf("\r\n");
				}
				element_index++;
...
compo_data_end:
	...
	if (ret == TRUE) {
		if (parse == FALSE) {
			printf("cfg_client_receive: opcode = 0x%x, len = %d, value = ", pmesh_msg->access_opcode,
				   pmesh_msg->msg_len);
			mesh_data_dump(pbuffer, pmesh_msg->msg_len);
		}

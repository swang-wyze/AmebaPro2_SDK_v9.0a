...
class Kconfig(object):
    ...
    def write_autoconf(self, filename=None, header=None, tail=None):
        r"""
        ...
        if filename is None:
            filename = os.getenv("KCONFIG_AUTOHEADER",
                                 "include/bt_autoconf.h")

        if self._write_if_changed(filename, self._autoconf_contents(header, tail)):
            return "Kconfig header saved to '{}'".format(filename)
        return "No change to Kconfig header in '{}'".format(filename)

    def _autoconf_contents(self, header, tail):

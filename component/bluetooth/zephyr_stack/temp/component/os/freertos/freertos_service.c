...
static int _freertos_sema_init_with_limit(_sema *sema, int init_val, int limit, char static_flag)
{
	if (!sema)
		return -1;

    if (static_flag)
        *sema = (void *)xSemaphoreCreateCountingStatic(limit, init_val, *sema);
    else
	    *sema = (void *)xSemaphoreCreateCounting(limit, init_val);

    return (*sema)?0:-1;
}

static u32 _freertos_sema_get_count(_sema sema)
{
	if (!sema)
		return 0;

    if (__in_interrupt())
        return (u32)uxQueueMessagesWaitingFromISR(sema);
    
	return (u32)uxSemaphoreGetCount(sema);
}

static int _freertos_mutex_init_static(_mutex *mutex)
{
    if (!mutex)
        return -1;

    *mutex = (void *)xSemaphoreCreateMutexStatic(*mutex);

    return (*mutex)?0:-1;
}

static int _freertos_spinlock_init_static(_lock *lock)
{
    if (!lock)
        return -1;

    *lock = (void *)xSemaphoreCreateMutexStatic(*lock);

    return (*lock)?0:-1;
}

static int _freertos_timerCreate_static(_timerHandle *timehandle, 
                                                    const char *pcTimerName, 
                                                    osdepTickType xTimerPeriodInTicks, 
                                                    u32 uxAutoReload, void *pvTimerID, 
                                                    TIMER_FUN pxCallbackFunction)
{
	if (xTimerPeriodInTicks == TIMER_MAX_DELAY)
		xTimerPeriodInTicks = portMAX_DELAY;

    *timehandle = xTimerCreateStatic(pcTimerName, xTimerPeriodInTicks, (BaseType_t)uxAutoReload,
                          (void *)pvTimerID, (TimerCallbackFunction_t)pxCallbackFunction, *timehandle);

    return (*timehandle)?0:-1;
}

static int _freertos_set_task_name(_thread_hdl_ task, const char* name)
{
    char* pname = pcTaskGetName(task);
    memcpy(pname, name, configMAX_TASK_NAME_LEN);
    return 0;
}

static void _freertos_set_scheduler_state(char enable)
{
    if (enable)
        vTaskStartScheduler();
    else
...
#ifdef CONFIG_USE_ZEPHYR_BT_STACK
	_freertos_sema_init_with_limit,	//rtw_init_sema_with_limit
	_freertos_sema_get_count,		//rtw_sema_get_count
	_freertos_mutex_init_static,    //rtw_mutex_init_static
	_freertos_spinlock_init_static, //rtw_spinlock_init_static
	_freertos_timerCreate_static,   //rtw_timerCreate_static
	_freertos_set_task_name,        //rtw_set_task_name
	_freertos_set_scheduler_state,  //rtw_set_scheduler_state
#endif
};

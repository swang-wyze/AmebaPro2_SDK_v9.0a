...
#ifdef CONFIG_USE_ZEPHYR_BT_STACK
int rtw_sema_init(_sema *sema, int init_val, int limit, char static_flag)
{
    if (!osdep_service.rtw_sema_init)
    {
        OSDEP_DBG("Not implement osdep service: rtw_init_sema");
        return -1;
    }

    return osdep_service.rtw_sema_init(sema, init_val, limit, static_flag);
}

u32 rtw_sema_get_count(_sema sema)
{
    if (!osdep_service.rtw_sema_get_count)
    {
        OSDEP_DBG("Not implement osdep service: rtw_sema_get_count");
        return 0;
    }

    return osdep_service.rtw_sema_get_count(sema);
}

int rtw_mutex_init_static(_mutex *mutex)
{
    if (!osdep_service.rtw_mutex_init_static)
    {
        OSDEP_DBG("Not implement osdep service: rtw_mutex_init_static");
        return -1;
    }

    return osdep_service.rtw_mutex_init_static(mutex);
}

int rtw_spinlock_init_static(_lock *lock)
{
    if (!osdep_service.rtw_spinlock_init_static)
    {
        OSDEP_DBG("Not implement osdep service: rtw_spinlock_init_static");
        return -1;
    }

    return osdep_service.rtw_spinlock_init_static(lock);
}

int rtw_timerCreate_static(_timerHandle *timehandle, const char *pcTimerName,
                                     osdepTickType xTimerPeriodInTicks, u32 uxAutoReload,
                                     void *pvTimerID, TIMER_FUN pxCallbackFunction)
{
    if (!osdep_service.rtw_timerCreate_static)
    {
        OSDEP_DBG("Not implement osdep service: rtw_timerCreate_static");
        return -1;
    }

    return osdep_service.rtw_timerCreate_static(timehandle, pcTimerName, 
                                                xTimerPeriodInTicks, uxAutoReload,
                                                pvTimerID, pxCallbackFunction);
}

int rtw_set_task_name(struct task_struct* task, const char* name)
{
     if (!osdep_service.rtw_set_task_name)
     {
         OSDEP_DBG("Not implement osdep service: rtw_set_task_name");
         return -1;
     }

    return osdep_service.rtw_set_task_name(task->task, name);
}

void rtw_set_scheduler_state(char enable)
{
     if (!osdep_service.rtw_set_scheduler_state)
     {
         OSDEP_DBG("Not implement osdep service: rtw_set_scheduler_state");
         return;
     }

    osdep_service.rtw_set_scheduler_state(enable);
}

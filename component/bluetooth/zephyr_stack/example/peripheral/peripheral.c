...
static struct bt_conn* peripheral_conn[CONFIG_BT_MAX_CONN] = {0};
...
static struct k_thread peripheral_thread;
static K_KERNEL_STACK_DEFINE(peripheral_thread_stack, 512);
...
static void connected(struct bt_conn *conn, uint8_t err)
{
	if (err) {
        ...
        for (uint8_t i = 0; i < CONFIG_BT_MAX_CONN; i++){
            if (0 == peripheral_conn[i]) {
                peripheral_conn[i] = conn;
                break;
            }
        }
	}
...
static void peripheral_thread_entry(void *p1, void *p2, void *p3)
{
    ARG_UNUSED(p1);
    ARG_UNUSED(p2);
    ARG_UNUSED(p3);
    ...
    bt_gatt_cb_unregister(&gatt_callbacks);

    err = bt_le_adv_stop();
    if (err) {
        printk("LE adv stop failed or has already stopped!");
    }

    /* Note: bt_le_adv_start use id(0) */
    bt_unpair(BT_ID_DEFAULT, NULL);

    for (uint8_t i = 0; i < CONFIG_BT_MAX_CONN; i++){
        if (peripheral_conn[i]) {
            err = bt_conn_disconnect(peripheral_conn[i], BT_HCI_ERR_REMOTE_USER_TERM_CONN);
            if (err) {
...
int example_bt_ze_peripheral(int argc, char **argv)
{
    ...
    en = atoi(argv[0]);
    if (en == 1) {
        if (peripheral_thread.run)
            return 0;

        if (!k_thread_create(&peripheral_thread, peripheral_thread_stack, 
                              K_KERNEL_STACK_SIZEOF(peripheral_thread_stack), 
                              peripheral_thread_entry, NULL, NULL, NULL, 5, 0, K_NO_WAIT)) {
            return -1;
        }
    } else if (en == 0) {
        if (peripheral_thread.run)
            k_thread_stop(&peripheral_thread, NULL, NULL);
        return 0;
    } else {
        return -EINVAL;
    }

...
/**
 * @brief Find a node from a list
 *
 * This and other sys_sflist_*() functions are not thread safe.
 *
 * @param list A pointer on the list to affect
 * @param node A pointer on the node to find from the list
 *
 * @return true if node was found
 */
static inline bool sys_sflist_find(sys_sflist_t *list,
					      sys_sfnode_t *node);

Z_GENLIST_FIND(sflist, sfnode)

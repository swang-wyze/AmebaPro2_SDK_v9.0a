...
extern bool z_is_inactive_timeout(struct _timeout *to);
extern int z_abort_timeout(struct _timeout *to);
extern int z_add_timeout(struct _timeout *to, _timeout_func_t fn, k_timeout_t timeout);
 ...
 * and pending_cancels.
 */
static struct k_spinlock lock = Z_SPINLOCK_INITIALIZER(lock);
...
static void work_queue_main(void *workq_ptr, void *p2, void *p3)
{
	...
	while (true) {
		...
		if (work == NULL) {
			 ...
			 */
			k_sem_take(&queue->notifyq, K_FOREVER);
            if (!queue->thread.run)
                break;
			...
			const struct k_work_queue_config *cfg)
{
	...
	(void)k_thread_create(&queue->thread, NULL, stack_size,
...
static void z_del_work_queue_thread(void* ctx)
{
    k_work_submit((struct k_work*)ctx);
}

void k_work_queue_stop(struct k_work_q *queue)
{
    __ASSERT_NO_MSG(queue);

    /* Delete work_q thread */
    struct k_work trigger_work;
    memset(&trigger_work, 0, sizeof(trigger_work));
    k_thread_stop(&queue->thread, z_del_work_queue_thread, &trigger_work);

    k_sem_free(&queue->notifyq);
 ...
 */
static void work_timeout(void* timerhandle)
{
	struct _timeout *to
		= CONTAINER_OF(timerhandle, struct _timeout, timerhandle);

...
static int cmd_unselect(const struct shell *shell, size_t argc, char *argv[])
{
	ARG_UNUSED(argc);
	ARG_UNUSED(argv);
	...
	if (default_conn) {
		...
		shell_error(shell, "No default connection found");
		return -ENOEXEC;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(bt_cmds,
	SHELL_CMD_ARG(init, NULL, "[no-settings-load], [sync]",
		      cmd_init, 1, 2),
	SHELL_CMD_ARG(deinit, NULL, "", cmd_deinit, 1, 0),
	...
	SHELL_CMD_ARG(unselect, NULL, HELP_ADDR_LE, cmd_unselect, 3, 0),

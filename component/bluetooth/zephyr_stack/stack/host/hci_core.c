...
static K_KERNEL_STACK_DEFINE(rx_thread_stack, CONFIG_BT_HCI_RX_STACK_SIZE);
#endif
			 ...
			 struct net_buf **rsp)
{
	...
	if (k_msg_send(&g_tx_message_queue, NULL, BT_EVENT_CMD_TX, K_NO_WAIT))
		BT_ERR("k_msg_send failed!");
...
static void hci_tx_thread(void *p1, void *p2, void *p3)
{
	...
	while (1)
	{
		if (k_msg_recv(&g_tx_message_queue, &tx_message, K_FOREVER) == 0)
		{
		    if (!tx_thread_data.run)
				break;

			if (BT_EVENT_CMD_TX == tx_message.tag) {
				send_cmd();
			} else if (IS_ENABLED(CONFIG_BT_CONN) | IS_ENABLED(CONFIG_BT_ISO)) {
				if (BT_EVENT_CONN_TX_QUEUE == tx_message.tag)
					bt_conn_process_tx((struct bt_conn *)tx_message.pointer);
                else
                    continue;
			}
		}
...
int bt_hci_driver_unregister(void)
{
	if (!bt_dev.drv)
		return 0;

    BT_DBG("Unregistering %s", bt_dev.drv->name ? bt_dev.drv->name : "");
	//bt_monitor_del_index(BT_MONITOR_TYPE_PRIMARY, drv->bus,
	//		     BT_ADDR_ANY, drv->name ? drv->name : "bt0");
...
static void hci_rx_thread(void)
{
	...
	while (rx_thread_data.run) {
		...
		case BT_BUF_EVT:
			...
			break;
        case 0xFF:
            /* This type is used as stop trigger. */
            break;
		default:
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	if (atomic_test_and_set_bit(bt_dev.flags, BT_DEV_ENABLE)) {
		BT_WARN("BT has already been enabled!");
		return 0;//return -EALREADY;
	}
...
int bt_disable(void)
{
	...
	if (!atomic_test_and_clear_bit(bt_dev.flags, BT_DEV_READY)) {
        BT_WARN("BT has already been unready!");
		return 0;//return -EALREADY;
    }

    /* TODO: Maybe Stop All Threads first! 
     * bt_disable flow now: 
     * HCI transport close -> bt_deinit -> HCI rx thread close -> rx thread close (if exist)
     * -> tx thread close -> work_q thread close -> deinit all related resources
     */
    if (bt_dev.drv) {
...
#if !defined(CONFIG_BT_RECV_IS_RX_THREAD)
    /* Delete RX thread */
    struct net_buf trigger_buf;
    memset(&trigger_buf, 0 ,sizeof(trigger_buf));
    k_thread_stop(&rx_thread_data, bt_del_hci_rx_thread, &trigger_buf);
    BT_INFO("BT RX thread exit");
#endif

	/* Delete TX thread */
	k_thread_stop(&tx_thread_data, bt_del_hci_tx_thread, NULL);
	BT_INFO("BT TX thread exit");

	err = bt_disable_dependence();
	if (err)
		return err;

		    ...
		    bt_conn_tx_cb_t cb, void *user_data)
{
    ...
    k_msg_send(&g_tx_message_queue, (void *)conn, BT_EVENT_CONN_TX_QUEUE, K_NO_WAIT);
	return 0;
}
...
void bt_conn_set_state(struct bt_conn *conn, bt_conn_state_t state)
{
	...
	case BT_CONN_CONNECTED:
		...
		//k_msg_send(&g_tx_message_queue, NULL, BT_EVENT_CONN_TX_QUEUE, K_NO_WAIT);
	...
	case BT_CONN_DISCONNECTED:
		...
		case BT_CONN_DISCONNECT_COMPLETE:
			...
			k_msg_send(&g_tx_message_queue, conn, BT_EVENT_CONN_TX_QUEUE, K_NO_WAIT);
...
static struct k_spinlock cb_lock = Z_SPINLOCK_INITIALIZER(cb_lock);
void bt_conn_cb_register(struct bt_conn_cb *cb)
{
	k_spinlock_key_t key = k_spin_lock(&cb_lock);
	cb->_next = callback_list;
	callback_list = cb;
	k_spin_unlock(&cb_lock, key);
}

void bt_conn_cb_unregister(struct bt_conn_cb *cb)
{
	struct bt_conn_cb *conn_cb;
	struct bt_conn_cb *conn_cb_prev;

	k_spinlock_key_t key = k_spin_lock(&cb_lock);
	for (conn_cb_prev = conn_cb = callback_list; conn_cb; conn_cb = conn_cb->_next) {
        if (cb == conn_cb) {
            if (conn_cb == conn_cb_prev) {
                callback_list = conn_cb->_next;
            } else {
                conn_cb_prev->_next = conn_cb->_next;
            }
            conn_cb->_next = 0;
            break;
        }
        conn_cb_prev = conn_cb;
	}
	k_spin_unlock(&cb_lock, key);
}
